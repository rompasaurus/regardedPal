...
// ============================================================
minus_x_inset_length_along_x_mm                = 9.5;
minus_x_inset_height_reduction_from_plug_mm    = 3.0;
...
// ============================================================
connecting_block_length_along_x_mm             = 9.5;
connecting_block_height_reduction_from_plug_mm = 3.0;
...
module aaa_cradle_insert() {
    ...
    difference() {
        ...
        // AAA cell bays — horizontal cylinders forming the bottom-half
        // trough each cell sits in. Each cell's bay is paired with a
        // top-open cut above (removes plug material from the cell's
        // equator up through the plug top) so the cell drops in from
        // above. -Y bay and +Y bay are pushed apart so each cell is
        // flush with the plug's long-side edge.
        for (bay_y_center_mm = [aaa_bay_minus_y_center_y_mm,
                                aaa_bay_plus_y_center_y_mm]) {
            // Cylinder cut (forms the full bay tunnel; top half gets
            // joined by the rectangular top-open cut below).
            ...
            // Top-open rectangular cut — spans the bay's XY bounding
            // box, from the cell's equator up to the plug top.
        ...
        // -X middle inset — 9.5 mm long pocket opening from the top of
        // the plug, on the -X end of the plug's long axis. 3 mm
        // shallower than the full plug (blind pocket, not through).
    ...
    // Connecting block — added AFTER the cuts so it survives inside
    // the Pico nest's footprint. Sits on the +X edge of the -X inset
    // and bridges the inset region into the rest of the plug.
