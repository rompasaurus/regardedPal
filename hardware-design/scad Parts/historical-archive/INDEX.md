# Historical SCAD Archive

Snapshots of every SCAD file in the Rev 2 enclosure family at the time it was last edited. Files are renamed with `<YYYY-MM-DD>_<HHMM>_<original-name>__<short-descriptor>.scad` so the folder reads as a chronological design history.

**Archive policy**: copies are append-only. To check what changed between two snapshots, `diff` two files in this folder. To recover a working part, copy the file out of this folder back into the live SCAD tree and rename it.

Each entry below lists:
- **What it is** — purpose / role in the enclosure stack
- **Footprint** — outer dimensions
- **Houses** — electronics expected to fit
- **Mates with** — other parts in the stack that pair with it
- **Distinguishing features** — what separates it from sibling files

---

## 2026-04-24_2110 — Live Rev 2 covers/base/middle (3-piece original lineage)

These are the parent-folder "live" files that the Bambu Studio prints came from. Outer footprint **91.5 × 44** (the original Rev 2 width before the dual-10440 redesign moved to 46 mm).

### `2026-04-24_2110_top-cover-windowed-v1__no-rails-blind-bores.scad`
- **What it is**: top cover with curved bullnose, display viewing window, joystick through-hole. No retention rails or lips inside — relies on a separate retention plate (not in this folder) to hold the display.
- **Footprint**: 91.5 × 44 × 14.5 (face plate + bullnose). Cover-local Z = 0 at mating bottom.
- **Houses**: 1× Waveshare 2.13" raw e-paper module, 1× joystick shaft (12 mm hole, tapers to 15 mm at top).
- **Mates with**: `middle-platform-v1.scad` below, then `base-v1.scad` underneath.
- **Distinguishing features**: 4 corner pillars run full-height up through the face plate; M3 clearance bores are blind from below (capped at face-plate bottom) so no holes are visible on the front face. Window length 48 × depth 27, shifted +X by 3 mm toward the joystick side to give a thicker bezel on the battery side.

### `2026-04-24_2110_top-cover-windowed-screen-inlay-v1__3mm-fpc-divet.scad`
...
- **Footprint**: same as v1 (91.5 × 44 × 14.5 → 14.7 after 0.5 → 0.7 mm face-plate thickness bump).
- **Houses**: same display + 1× custom joystick PCB (20 × 20, with 15 × 15 mm M3 hole pattern, joystick component sits 0.65 mm +X of PCB center).
- **Mates with**: same as v1 — `middle-platform-v1` + `base-v1`.
- **Distinguishing features**: 3 mm screen inlay recess + **3 mm**-deep FPC ribbon divet on the -X wall (thins it from 3 mm to 1 mm in a 13 mm-wide Y band so the display's FPC can exit toward the battery end). Joystick PCB pocket aligned flush with inlay's +X edge (so no thin sliver of uncut material between them).

### `2026-04-24_2110_top-cover-windowed-screen-inlay-v1-2mm__2mm-fpc-divet.scad`
...
### `2026-04-24_2110_base-v1__1000mAh-flat-cell-44y.scad`
...
- **Distinguishing features**: -Y-edge battery-floor pit (10 × 35.5 × 2 mm down into the floor plate) for a battery-side connector; battery-stop pillars on the -X face of the divider; 4 corner pillars with M3 through-bolts; BOOT/RST paperclip holes through the floor under the ESP32.

### `2026-04-24_2110_middle-platform-v1__44y-with-pedestal.scad`
...
- **Houses**: no electronics directly. Provides display-support pedestal on the battery half + open chamber on the ESP32 half.
...
## 2026-04-24_2124 — Alteration fork (dual-10440, 46 mm Y, 2-piece designs)

These live in `04-24-designs-alterations/` and represent a redesign session: outer width grew from 44 → **46 mm** to accommodate two flanking 10440 Li-ion cells; later iterations dropped the middle platform entirely.

### `2026-04-24_2124_top-cover-windowed-v1__alteration-fork.scad`
- **What it is**: copy of the live `top-cover-windowed-v1` with experimental tweaks made in the alterations folder. Functionally identical to its parent at the time of fork; kept in case the alteration tweaks are needed.
...
### `2026-04-24_2124_top-cover-windowed-screen-inlay-pico2-v2-2mm__pico2-board-46y.scad`
...
- **Mates with**: `base-v3-2piece` / `base-v4-2piece-open` (or a Pico-2-specific base if/when one is forked).
- **Distinguishing features**: same outer geometry as the v2-46y inlay cover; only inner pocket dimensions and Pico cutout differ to suit the Pico 2's footprint.

### `2026-04-24_2124_top-cover-windowed-screen-inlay-v2-46y__dual-10440-46y.scad`
- **What it is**: the inlay cover redesigned for the dual-10440 case (46 mm Y instead of 44).
- **Footprint**: 91.5 × 46 × 14.5.
- **Houses**: 1× Waveshare 2.13", 1× custom joystick PCB; the cells/Pico are in the base, not the cover.
...
### `2026-04-24_2124_top-cover-windowed-screen-inlay-v3-2piece__no-mid-platform-46y.scad`
...
### `2026-04-24_2124_base-v2-thin-dual-10440__46y-flanking-cells.scad`
- **What it is**: redesigned base for two flanking 10440 cells with a Pico W down the centerline.
- **Footprint**: 91.5 × 46 × 12 (lower than base-v1 because the flat cell's height drove that, and 10440s lay on their side).
...
### `2026-04-24_2124_base-v3-2piece__absorbs-mid-platform-46y.scad`
- **What it is**: 2-piece variant of base-v2 that grows in Z from 12 → **14** to absorb the middle-platform's display-support role. Stack collapses to base + cover, no mid-piece.
...
- **Distinguishing features**: cell bay ceiling stays at z = 12 (cells stay trapped at Z = 10 + 2 floor plate); the 12 → 14 mm growth is pure rim material above the cells. Pico W chamber centered in Y (12.1 → 33.9) extends from shelf top (z = 8) to case top (z = 14).

### `2026-04-24_2124_base-v4-2piece-open__slot-load-batteries-46y.scad`
...
- **Houses**: same.
- **Mates with**: same as base-v3 — `top-cover-windowed-screen-inlay-v3-2piece`.
...
### `2026-04-24_2124_middle-platform-v2-46y__46y-pedestal-variant.scad`
...
## 2026-04-24_2333 — AAA cradle insert

### `2026-04-24_2333_aaa-cradle-insert-v1__2x-aaa-pico-nest-46y.scad`
...
- **Mates with**: `top-cover-windowed-screen-inlay-v3-2piece` above + (later) `base-plate-v1.scad` below.
...
## 2026-04-25_0035 — Base plate

### `2026-04-25_0035_base-plate-v1__snap-pegs-46y.scad`
...
| Need | Action |
|---|---|
| Look at what changed between two snapshots | `diff` two files in this folder |
| Recover a working part from a date | Copy the file out, drop the `<date>_<time>_` and `__<descriptor>` parts of the name, paste it back into the live SCAD tree |
| Add a new snapshot | Once you make a meaningful change to a SCAD, copy it here with a fresh `<YYYY-MM-DD>_<HHMM>_<original>__<descriptor>.scad` name and add an entry to this INDEX |
| Verify the archive matches reality | Run `openscad -o /tmp/x.stl <archive-file>.scad` — every file should still render |

The chronological prefix is intentional: filenames sort newest-last, so `ls -1 historical-archive/` reads as a top-down design history.
