// FINAL — BASE PLATE WITH SOLAR CUTOUT
//
// Copy of base-plate-solar-usb-clip-fix-27-04.scad with the
// "extended_inset-pillar-blocks" preset fully baked in.
//
// Preset diff vs. SCAD defaults:
//   - pillar_extension_x_inward_uniform_mm   5 -> 27
//
// Export:
//   openscad -o base-plate-final.3mf base-plate-final.scad

$fn = 48;

// ============================================================
// Outer shell — match the cover / cradle 91.5 x 46 footprint so
...
// plug_x_start/end and plug_y_start/end (3.3–90.0 x 2.3–43.7).
// ============================================================
...
// ============================================================
pillar_extension_x_inward_uniform_mm           = 27;    // baked from "extended_inset-pillar-blocks" preset (was 5)
...
// Standard USB-C receptacle: 8.94 mm wide x 3.26 mm tall.
...
usb_c_cutout_corner_r_mm                       = 1.0;     // actual USB-C ~0.8 mm + print tolerance (was 1.73 stadium)
...
module base_plate_v1_unshaved() {
    ...
    union() {
        ...
        difference() {
            ...
            // USB-C port notch on +X wall — minkowski(cube + sphere)
            // for uniform 1 mm rounding on all edges. Inner cube is
            // shrunk by r on each rounded axis; sphere adds it back.
            // Top overshoots past the mating surface so it's open.
            usb_c_r = usb_c_cutout_corner_r_mm;
            translate([usb_c_wall_x_start_mm - 0.1,
                       usb_c_cutout_center_y_mm - usb_c_cutout_width_y_mm / 2 + usb_c_r,
                       usb_c_cutout_z_bottom_mm + usb_c_r])
            minkowski() {
                cube([plus_x_end_wall_thickness_mm + 0.2,
                      usb_c_cutout_width_y_mm - 2 * usb_c_r,
                      usb_c_cutout_height_z_mm - usb_c_r + 0.1]);
                sphere(r = usb_c_r, $fn = 48);
            }
        ...
        // CORNER (the original 5x5 mm pad) into the cover's blind M3
        ...
        for (i = [0 : len(peg_xy_positions_list) - 1]) {
            ...
            // Original 5x5 screw pillar — always pocket floor to plate top.
