// FINAL — AAA CRADLE WITH BATTERY CLIPS (2026-04-29)
//
// Copy of aaa-cradle-with-batt-clips.scad from Working Model Rev 1,
// with the "battery clips first print 04-27 Shortened holes by 5mm
// and opened batt clip window at top" preset values baked in:
//   - aaa_cell_length_mm = 39.5  (was 44.5)
//   - batt_clip_retainer_window_z_extend_up_mm = 4  (was 0.0)
//
// All other values are identical to the parent SCAD.
//
...
// battery contact plate (unipolar, 7 x 7 mm stamped sheet metal),
// one at each X end of each AAA bay.
//
// How the slots are used:
//   - Each plate drops in from the plug top face into a 7.4 x 7.4 mm
...
//     before insertion.
//
// Note: only unipolar plates are used (4 total = 2 bays x 2 ends).
// Wire the cells in PARALLEL — never in series, since 2S 10440 = 7.4 V
// would exceed the Pico W's 5.5 V VSYS max.
//
// Export:
//   openscad -o aaa-cradle-final.3mf aaa-cradle-final.scad
...
// ============================================================
aaa_cell_length_mm                             = 39.5;   // BAKED: was 44.5 — shortened 5 mm for 04-27 print
...
aaa_bay_minus_x_start_mm =
    aaa_bay_plus_x_end_mm - aaa_bay_length_along_x_mm;   // = 25.4
...
batt_clip_retainer_window_z_extend_up_mm       = 4;       // BAKED: was 0.0 — opened batt clip window at top for 04-27 print
